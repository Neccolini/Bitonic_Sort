...
mod tests{
    ...
    //use crate::utils::{new_uew_vec,is_sorted_ascending,is_sorted_descending};
...
/*
    ...
    #[test]
    fn sort_students_by_age_ascending(){
        let mike=Student::new("Misaki","Akeno",16);
        let shiro=Student::new("Mashiro","Muneya",14);
        let tama=Student::new("Shima","Tateishi",18);
        let mei=Student::new("Mei","Irizaki",15);
        let rin=Student::new("Rin","Shiretoko",17);
        let mut x=vec![&mike,&shiro,&tama,&mei,&rin];
        assert_eq!(sort_by(&mut x,
            &|a, b| a.age.cmp(&b.age)
                .then_with(|| a.first_name.cmp(&b.first_name))), Ok(())
        );
        let expected=vec![&shiro,&mei,&mike,&rin,&tama];
        assert_eq!(x,expected);
    }
*/
}